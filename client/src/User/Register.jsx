  ...
  const onRegistration = e =>{
    if(!userName) setUserNameError("Your name is required");
      ...
      postToServer(
        ...
        setPassword('');
        setConfirmPassword('');
      }).catch(text=>toast.error(text.status)).finally(()=>setLoading(false));
    }
  }
  ...
  return <div className='d-flex flex-column justify-content-center align-items-center' style={{height:"100%",backgroundImage:`url("/assets/images/login_background.jpg")`,backgroundSize:'100% 100%'}}>
    <div className='d-flex flex-column align-items-center justify-content-center flex-grow-1' style={{width:"360px"}}>
      ...
      <div className={`mb-3 d-flex align-items-center justify-content-between input-group form-control border`} >
        <input type="email" className="border-0 flex-grow-1" value={userName} onChange={e=>{setUserName(e.target.value);setUserNameError('')}} placeholder="User name" style={{height:"45px",outline:"none"}}/>
        <span className="material-symbols-outlined">person</span>
      </div>
      {userNameError && <p className='d-flex align-items-center' style={{color:"red",width:"100%"}}>
          <span className="material-symbols-outlined me-1" style={{color:"red"}}>error</span>
          {userNameError}
        </p>
      }
      <div className={`mb-3 d-flex align-items-center justify-content-between input-group border form-control border`}>
        <input type="password" className="border-0 flex-grow-1" value={password} onChange={e=>{setPassword(e.target.value);setPasswordError('')}} placeholder="Password" style={{height:"45px",outline:"none"}}/>
        <span className="material-symbols-outlined">password</span>
      </div>
      {passwordError && <p className='d-flex align-items-center' style={{color:"red",width:"100%"}}>
          <span className="material-symbols-outlined me-1" style={{color:"red"}}>error</span>
          {passwordError}
        </p>
      }
      <div className={`mb-3 d-flex align-items-center justify-content-between input-group border form-control border`}>
        <input type="password" className="border-0 flex-grow-1" value={confirmPassword} onChange={e=>{setConfirmPassword(e.target.value);setConfirmPasswordError('')}} placeholder="Re-enter Password" style={{height:"45px",outline:"none"}}/>
        <span className="material-symbols-outlined">password</span>
      </div>
      {confirmPasswordError && <p className='d-flex align-items-center' style={{color:"red",width:"100%"}}>
          <span className="material-symbols-outlined me-1" style={{color:"red"}}>error</span>
          {confirmPasswordError}
        </p>
      }
      {loading?<div className='spinner-grow'></div>:
        <ButtonComponent label="sign up" onClick={onRegistration}/>
      }
